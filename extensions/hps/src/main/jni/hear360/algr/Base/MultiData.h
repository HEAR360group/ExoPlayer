...
inline static void SetMonoWithStride (float *pdstaudio, const float *value, long totalsamples, long stride, long offset)
{
  for (long sx = 0 ; sx < totalsamples ; sx++)
  {
    int index = sx * stride + offset;
    pdstaudio[index] = *value;
  }
...
inline static void CopyMonoSIMDWithStride (float *pdstaudio, const float *psrcaudio, long totalsamples, long dstStride, long srcStride, long dstOffset, long srcOffset)
{
  long framesProduced = 0;

#if defined(__APPLE__)
  float zero = 0;
  vDSP_vsadd(psrcaudio + srcOffset, srcStride, &zero, pdstaudio + dstOffset, dstStride, totalsamples);
  return;
#endif

  for (long sx = framesProduced ; sx < totalsamples ; sx++)
  {
    long dstIndex = sx * dstStride + dstOffset;
    long srcIndex = sx * srcStride + srcOffset;
    pdstaudio[dstIndex] = psrcaudio[srcIndex];
  }
}

inline static void CopyMonoWithStride (float *pdstaudio, const float *psrcaudio, long totalsamples, long dstStride, long srcStride, long dstOffset, long srcOffset)
{
  for (long sx = 0 ; sx < totalsamples ; sx++)
  {
    long dstIndex = sx * dstStride + dstOffset;
...
inline static void SubMonoSIMD (float *pdstaudio, const float *psrc1audio, const float *psrc2audio, long totalsamples)
{
  long framesProduced = 0;

  #if defined(__APPLE__)
    vDSP_vsub(psrc1audio, 1, psrc2audio, 1, pdstaudio, 1, totalsamples);
    return;
  #elif SIMD_OPTI
  long framesSIMD = totalsamples - totalsamples % 4;
  ...
  while (framesProduced < framesSIMD)
  {
    AKSIMD_V4F32 vIn1 = AKSIMD_LOAD_V4F32(pfIn1);
    AKSIMD_V4F32 vIn2 = AKSIMD_LOAD_V4F32(pfIn2);
    AKSIMD_V4F32 vOut = AKSIMD_SUB_V4F32(vIn1, vIn2);
    AKSIMD_STORE_V4F32(pfOut, vOut);
    pfIn1 += 4;
    pfIn2 += 4;
    pfOut += 4;
...
inline static void SubMono (float *pdstaudio, const float *psrc1audio, const float *psrc2audio, long totalsamples)
{
  for (long sx = 0 ; sx < totalsamples ; sx++)
  {
    pdstaudio[sx] = psrc1audio[sx] - psrc2audio[sx];
...
inline static void MulMonoByScalarSIMD(float *pdstaudio, const float *psrcaudio, const float scalar, long totalsamples)
{
  long framesProduced = 0;

#if defined(__APPLE__)
  vDSP_vsmul(psrcaudio, 1, &scalar, pdstaudio, 1, totalsamples);
  return;
#elif SIMD_OPTI
  long framesSIMD = totalsamples - totalsamples % 4;
  ...
  while (framesProduced < framesSIMD)
  {
    AKSIMD_V4F32 vIn = AKSIMD_LOAD_V4F32(pfIn);
    AKSIMD_V4F32 vOut = AKSIMD_LOAD_V4F32(pfOut);
    vOut = AKSIMD_MUL_V4F32(vIn, vScalar);
    AKSIMD_STORE_V4F32(pfOut, vOut);
    pfIn += 4;
    pfOut += 4;
    framesProduced += 4;
...
inline static void MulMonoByScalarSIMDWithStride(float *pdstaudio, const float *psrcaudio, const float scalar, long totalsamples, long dstStride, long srcStride, long dstOffset, long srcOffset)
{
  long framesProduced = 0;

#if defined(__APPLE__)
  vDSP_vsmul(psrcaudio + srcOffset, srcStride, &scalar, pdstaudio + dstOffset, dstStride, totalsamples);
  return;
#endif
  ...
  for (long sx = framesProduced ; sx < totalsamples ; sx++)
  {
    long dstIndex = sx * dstStride + dstOffset;
    long srcIndex = sx * srcStride + srcOffset;
    pdstaudio[dstIndex] = (psrcaudio[srcIndex] * scalar);
  }
}

inline static void MulMonoByScalar(float *pdstaudio, const float *psrcaudio, const float scalar, long totalsamples)
{
  for (long sx = 0 ; sx < totalsamples ; sx++)
  {
    pdstaudio[sx] = (psrcaudio[sx] * scalar);
...
inline static void LimitMonoSIMD (float *pdstaudio, const float *psrcaudio, const float min, const float max, long totalsamples)
{
#if defined(__APPLE__)
  vDSP_vclip(psrcaudio, 1, &min, &max, pdstaudio, 1, totalsamples);
  return;
#endif

  for (long sx = 0 ; sx < totalsamples ; sx++)
  {
    float src = psrcaudio[sx];
