...
)
{
  ...
  while (currentposition < totalsamples)
  {
    ...
    pshifteddstaudiointerleaved = pdstaudiointerleaved + (currentposition * totaldstchannels);
...
*/
    ProcessLow (pprivate, pshifteddstaudiointerleaved, pshiftedsrcaudiointerleaved, totaldstchannels, totalsrcchannels, samplestoprocess);
...
bool LoadIRsFromFloats(void* handle, float** leftIRs, float** rightIRs, bool enableEQ, float masterGain, float* gains, unsigned int irFrames)
{
  ...
  pprivate->processor.LoadIRsFromFloats(leftIRs, rightIRs, enableEQ, masterGain, gains, irFrames);
...
bool LoadIRsFromInts(void* handle, int** leftIRs, int** rightIRs, bool enableEQ, float masterGain, float* gains, unsigned int irFrames)
{
  ...
  pprivate->processor.LoadIRsFromInts(leftIRs, rightIRs, enableEQ, masterGain, gains, irFrames);
...
bool ProcessOutOfPlaceInterleaved(void* handle, hear360_dsp_high_hrirfolddownsimple::PARAMETERS *pparameters, const float* pInBuf, float* pOutbuf, int srcChannels, int dstChannels, long totalsamples)
{
  ...
  ProcessInBlocks (pprivate, pOutbuf, pInBuf, dstChannels, srcChannels, totalsamples);
