...
void CRFilter::loadIRFromFloats(float* leftIRs, float* rightIRs, unsigned int irFrames)
{
    InitIRsMem(irFrames);

    for(unsigned int i = 0; i < irFrames; i++)
    {
        mIRFullLeft[i] = leftIRs[i] / mTotalIrPages;
        mIRFullRight[i] = rightIRs[i] / mTotalIrPages;
    }
