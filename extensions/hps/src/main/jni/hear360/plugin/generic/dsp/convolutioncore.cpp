...
void loadIRs(void *handle, unsigned int channelID, int presetID)
{
  PRIVATE* pprivate = (PRIVATE*)handle;
  pprivate->processor.LoadIRs (channelID, pprivate->mSamplerate, presetID);
}

void loadIRFromFloats(void *handle, float* leftIRs, float* rightIRs, int irFrames)
{
  PRIVATE* pprivate = (PRIVATE*)handle;
  pprivate->processor.LoadIRFromFloats (leftIRs, rightIRs, irFrames);
}
